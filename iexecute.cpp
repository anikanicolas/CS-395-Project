...
std::vector<std::string> iexecute(const std::vector<std::string> &decoded, const uint32_t &pc) {
    ...
    if (!decoded.empty()) {
        ...
        if (decoded.size() > 4) {
            arg3 = decoded[4];  // This value is never used
        }
        ...
        if (decoded == stalling) {
            ...
            if (op == "LUI") {
                /*
                 * A LUI instruction can first load arg1 with the upper 20 bits
                 * of a target address LUI (load upper immediate) places the
                 * 20-bit U-immediate into bits 31–12 of register arg0 and
                 * places zero in the lowest 12 bits.
                 */
                ...
                target address JAL stores the address of the instruction
                ...
                */
                ...
                // loads an 8-bit value from memory, then sign-extends to
                ...
                // loads 8-bit values from memory but then zero extends to
                // 32-bits before storing in rd
                return {"LBU", arg0, std::to_string(stoul(arg2) + stoul(arg2))};
            } else if (op == "LHU") {
                // loads a 16-bit value from memory but then zero extends to
                // 32-bits before storing in rd
                 ...
                 * addresses and uses the U-type format. AUIPC forms a 32-bit
                 * offset from the 20-bit U-immediate, filling in the lowest 12
                 * bits with zeros, adds this offset to the pc, then places the
                ...
                /*
                 * SLT and SLTU
                 * perform signed and unsigned compares respectively, writing 1
                 * to rd if rs1 < rs2, 0 otherwise. Note, SLTU rd, x0, rs2 sets
                 * rd to 1 if rs2 is not equal to zero, otherwise sets rd to
